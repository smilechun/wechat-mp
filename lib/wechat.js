...
Wechat.prototype.parser = function bodyParser(opts) {
  ...
  return function(req, res, next) {
    ...
    if (!qs.echostr || !qs.nonce || !qs.signature || !qs.timestamp)
      return next()
...
//By Fat
Wechat.prototype.requestParser = function (req, callback) {
  var opts =  {}
  defaults(opts, this.options)
  ...
  // TODO use a special property to demine whether this is a wechat message
  ...
  if (!checkSig(token, req.query)) {
    ...
    Wechat.parse(req, function(err, data) {
      ...
      if (generateSid) {
        ...
        Object.defineProperty(data, 'sessionID', propdef)
        Object.defineProperty(data, 'sessionId', propdef)
      }
      callback(null, data);
    })
...
Wechat.prototype.requestResponder = function (req, res) {
  if (!res.headersSent) {
    res.setHeader('Content-Type', 'application/xml')
    res.end(Wechat.dump(Wechat.ensure(res.body, req.body)))
  }
